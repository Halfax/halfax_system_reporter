...
def run_command(command, **kwargs):
    """Run a command with non-interactive sudo to avoid password hangs in headless/report paths."""
    if isinstance(command, (list, tuple)) and command and command[0] == 'sudo' and '-n' not in command:
...
def get_system_info(interactive=False, sudo_password=None):
    """Get comprehensive system information from multiple methods
    
    Args:
        interactive: If True, can prompt user for sudo password and attempt privileged operations
        sudo_password: Optional pre-provided sudo password (for automation)
    
    Returns:
        dict with system info and optional '_collection_failures' list
    """
    system_info = {
    ...
    if IS_WINDOWS and HAS_WMI:
        ...
        try:
            ...
            result = run_command(['sudo', '-n', 'dmidecode', '-s', 'system-product-name'], 
                                 capture_output=True, text=True, timeout=5)
            if result.returncode == 0:
                ...
                try:
                    ...
                    if pw:
                        cmd = 'echo "{}" | sudo -S dmidecode -s system-product-name'.format(pw)
                        result = run_command(cmd, capture_output=True, text=True, timeout=5, shell=True)
                        if result.returncode == 0:
                            system_info['model'] = result.stdout.strip() or 'Unknown'
                        else:
                            collection_failures.append('System model (sudo access denied)')
                    else:
                        collection_failures.append('System model (no password provided)')
                except Exception:
                    collection_failures.append('System model (collection failed)')
            else:
                collection_failures.append('System model (sudo access required)')
            
            # Try to get serial
            result = run_command(['sudo', '-n', 'dmidecode', '-s', 'system-serial-number'], 
                                 capture_output=True, text=True, timeout=5)
            if result.returncode == 0:
                ...
                try:
                    ...
                    pw = getpass.getpass('[sudo] password: ')
                    if pw:
                        cmd = 'echo "{}" | sudo -S dmidecode -s system-serial-number'.format(pw)
                        result = run_command(cmd, capture_output=True, text=True, timeout=5, shell=True)
                        if result.returncode == 0:
                            system_info['serial'] = result.stdout.strip() or 'Unknown'
                        else:
                            collection_failures.append('System serial (sudo access denied)')
                    else:
                        collection_failures.append('System serial (no password provided)')
                except Exception:
                    collection_failures.append('System serial (collection failed)')
            else:
                collection_failures.append('System serial (sudo access required)')
        except Exception:
            pass
    ...
    if collection_failures:
        system_info['_collection_failures'] = collection_failures
